 ...
 try {
  ...
  try {
   ...
   if (passDecrypted === pass) {
    const token = jsonWebToken.sign({id:user.id, username:user.username}, process.env.SECRET);
    if (req.signedCookies.token) {
...
app.post("/register", logs,  async (req, res) => {
 try {
  ...
  const create = await prisma.user.create({data:{username:data.username,password:encrypt(data.password), email:data.email, role: "USER"}});
...
app.post("/generate", [logs, auth], async (req, res) => {
 ...
 if (req.signedCookies.token) {
  ...
  await prisma.post.create({data:{authorId:data.id, title:data2.title, content:data2.description, published:true}});
  res.json("Submitted");
 }
});
